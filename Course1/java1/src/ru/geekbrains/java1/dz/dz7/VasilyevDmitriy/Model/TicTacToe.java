...
public class TicTacToe {
    ...
    public static final int SIZE = GameMap.getGameSize(); //Размер игрового поля Рекомендуется от 3 до 9
    private static char[][] map = new char[SIZE][SIZE]; // игровое поле

    public static char[][] initMap() {
        char[][] maps = new char[SIZE][SIZE];// инициализируем массив map(игровое поле)
        for (int i = 0; i < SIZE; i++) { // двойным циклом проходимся по всему массиву
            for (int j = 0; j < SIZE; j++) {
                maps[i][j] = '*'; // и заполняем каждую ячейку *
            }
        }
        return maps;
    }
    ...
    //Метод, возвращающий координаты для предупреждения победы игрока или поиска собственного победного хода
        public static String predictWin(char map[][],char x) {
...
/*        Идем по строке и столбцу. Если встречаем Х то коэффициент увеличиваем на 1
    ...
    public static boolean checkWin(char map[][],char ox) { // Проверяем победу
    ...
    public static boolean isCellEmpty(char map[][],int x, int y) { // проверка является ли ячейка свободной
